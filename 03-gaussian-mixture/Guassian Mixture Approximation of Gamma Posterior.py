...
Sample θ ∼ Gamma(α=2, β=3) and then y = ∑ₙPoisson(θ) over n=100 trials.
Build a MixtureDensityNN(K=8) via modles.get_model('mog',8).
Train it by minimizing the negative mixture log-likelihood of θ|y across all y∈[0..100].
Train a single-Gaussian ProbabilisticNN (K=3) similarly.
At the end, for a chosen y_obs=70, plot the true Gamma posterior vs the MoG and Gaussian approximations.

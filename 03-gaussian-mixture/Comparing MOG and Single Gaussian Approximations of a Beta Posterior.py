...
θ ∼ Beta(1,1), y ∼ Binomial(n=100, θ), M=10⁶ sims.
Use modles.get_model to instantiate both a MoG (K=8) and a Gaussian (K=3) net.
Train each via their log-likelihood losses on θ|y for all y.
Call plot_posterior_comparisons to overlay the true Beta(α+y,β+n–y) posterior with both NN approximations for a grid of y’s.

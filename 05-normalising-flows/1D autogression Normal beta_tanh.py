"""
Defines a 1D unconditional normalizing flow using a learnable affine + tanh transform.
  - Samples z ∼ N(0,1)
  - Transforms x = tanh(a·z + b)
  - Uses inverse tanh and Jacobian for log-density
  - Trains (a,b) by minimizing NLL against Beta(2,5)
  - Visualizes learned vs. true Beta PDF
"""
...
class InvertibleTanhAffineTransform(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, z):
        return torch.tanh(self.a * z + self.b)

    def inverse(self, x):
        x = x.clamp(-0.999, 0.999)  # 避免 log(0) 溢出
        return (0.5 * torch.log((1 + x) / (1 - x)) - self.b) / self.a

    def log_abs_det_jacobian(self, x):
        z = self.inverse(x)
        t = self.a * z + self.b
        return torch.log(torch.abs(self.a) * (1 - torch.tanh(t) ** 2) + 1e-8)
...
# Transform
transform = InvertibleTanhAffineTransform()
